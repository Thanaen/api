...
import { mapToMovie } from "../src/modules/cinema/service";
...
describe("mapToMovie", () => {
  const movie = mapToMovie(moviesFixture[0] as Parameters<typeof mapToMovie>[0]);
  ...
  test("maps poster", () => {
    expect(movie.poster).toContain("acsta.net");
  });

  test("maps releaseDate", () => {
    expect(movie.releaseDate).toBe("2026-02-04T00:00:00.000Z");
  });

  test("maps runtime", () => {
    expect(movie.runtime).toBe(5940);
  });

  test("maps production from studio.name", () => {
    expect(movie.production).toBe("Pathé Films");
  });

  test("maps casting array", () => {
    expect(movie.casting).toEqual(["Philippe Lacheau", "Jamel Debbouze", "Élodie Fontan"]);
  });

  test("maps direction array", () => {
    expect(movie.direction).toEqual(["Philippe Lacheau"]);
  });

  test("maps synopsis from locale", () => {
    expect(movie.synopsis).toBe("Pour sauver son emploi, David accepte un plan foireux.");
  });

  test("handles missing optional fields gracefully", () => {
    const minimal = mapToMovie({
      id: "999",
      title: "Test",
      runtime: 0,
      genres: "",
